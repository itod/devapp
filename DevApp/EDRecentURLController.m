...
@interface EDRecentURLController () 
- (void)addBaseURLForURLString:(NSString *)URLString;
- (void)checkForOverTheLimit;
@end

@implementation EDRecentURLController

+ (EDRecentURLController *)instance {
    EDAssertMainThread();
    static EDRecentURLController *instance = nil;
    if (!instance) {
        instance = [[EDRecentURLController alloc] init];
    }
    return instance;
}
...
//        [_recentURLs addObjectsFromArray:storedURLs];
...
- (void)addRecentURL:(NSString *)URLString {
    if ([URLString length]) {
        ...
        if (![_recentURLs containsObject:URLString]) {
...
- (void)checkForOverTheLimit {
    if ([_recentURLs count] > URL_LIMIT) {
        //NSLog(@"over the limit!!!!");
        NSInteger i = 0;
        for ( ; i < 30; i++) {
            //NSLog(@"evicting: %@", [_recentURLs objectAtIndex:0]);
            [_recentURLs removeObjectAtIndex:0];
        }
    }
}
...
- (void)addMatchingRecentURL:(NSString *)URLString {
    if ([URLString length] && ![_matchingRecentURLs containsObject:URLString]) {
        [_matchingRecentURLs addObject:URLString];
        [_matchingRecentURLs sortUsingSelector:@selector(caseInsensitiveCompare:)];
    }
}


- (void)save {
//    [[FUUserDefaults instance] setRecentURLStrings:_recentURLs];
